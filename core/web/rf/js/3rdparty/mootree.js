...
Element.NativeEvents.shiftclick=2;
...
var MooTreeControl = new Class({
	...
	initialize: function(config, options) {
		...
		this.theme = config.theme || './img/3rdparty/mt/mootree.gif';
		
		this.loader = config.loader || {icon:'./img/3rdparty/mt/mootree_loader.gif', text:'Loading...', color:'#a0a0a0'};
		
		this.selected = null;                 // set the currently selected node to nothing
		this.multipleselected = new Array();
		...
		this.onMultiSelect = config.onMultiSelect || new Function(); // called when any node in the tree is shift-clicked
	...
	*/
	...
	select: function(node) {
		// Multiselect: Holding down Shift Key
		if (this.multi) {
			this.multi=false;
			// Return if the selected one has a different parent
			node.selected = node.selected?false:true;
			node.update();
			if (node.selected) this.multipleselected.include(node);
			else this.multipleselected.erase(node);
			// Select multiples only if the parents are the same.
			if (node.parent == this.multipleselected.pick().parent) {
				// Fire multi event only if more than one selected
				if (this.multipleselected.length>1) this.onMultiSelect(this.multipleselected, true);
				// Return
				return;
			}
		}
		...
		// Deselect Multiselect & Empty it.
		this.multipleselected.each(function(item){
			if (item != node) {
				item.selected = false;
				item.update();
			}
		});
		this.multipleselected.empty();
		this.onMultiSelect(this.multipleselected, false);		
		...
		this.multipleselected.include(node);
	},
...
var MooTreeNode = new Class({
	...
	initialize: function(options) {
		...
		this.cssclass = options.cssclass || null; // custom css class of the node
		this.preselected = options.preselected || null; // custom css class of the node		
		this.forceReload = options.forceReload ? true : false;   // force xml-reload on every toggle
		...
		this.onMultiSelect = options.onMultiSelect || new Function(); // called when the individual node is selected/deselected		
		...
		this.div.gadget.onclick = this.div.gadget.ondblclick = this.div.icon.ondblclick = this.div.text.ondblclick = function() {
			this._node.toggle();
		}
		
		// attach event handler to icon/text:
		this.div.icon.addEvent('shiftclick', function(e){this._node.control.multiselect(this._node);});
		this.div.text.addEvent('shiftclick', function(e){this._node.control.multiselect(this._node);});

		this.div.icon._node = this.div.text._node = this;
		this.div.icon.onclick = this.div.icon.onclick = this.div.text.onclick = function() {this._node.control.select(this._node);}
	...
	Parameters:
		node - Node in the tree, possibliy a parent
	...
	*/
	...
	/*
	Property: selectSibling
		Select and Expands the previous or next sibling
	
	Parameters:
		prevnext - "Previous" or "Next"

	Returns:
		A <MooTreeNode> instance.
	*/
	selectSibling: function(prevnex) {
		var actIndex;
		var node = 0;
		this.parent.nodes.each(function(n,index) {
			if (n.open) {
				actIndex = index;
			}
		});
		if (prevnex=="Next") {
			if (this.parent.nodes[actIndex+1]!=null) node = 1;
		}
		if (prevnex=="Previous") {
			if (this.parent.nodes[actIndex-1]!=null) node = -1;			
		}		
		this.toggle(false);
		this.parent.nodes[actIndex+node].toggle(false);
		this.control.select(this.parent.nodes[actIndex+node]);
		return this.parent.nodes[actIndex+node];
	},
	...
	*/
	...
	update: function(recursive, invalidated) {
		...
		if (draw) {
			...
			var x;
			// Trigger select if preselect is set in the xml file
			if (this.preselected) {
			...
			if (this.cssclass) x.addClass(this.cssclass);
	...
	*/
	...
	getImg: function(name) {
		...
		if (name != '') {
			...
			html += ' style="width: 16px; background-image:url(' + img + '); background-position:-' + (i*18) + 'px 0px;"';
		}
		else {
	...
	*/
	...
	/*
	Property: reload
		Forces a reload and opens a branch. Works only on nodes with a load parameter set.
		
	Parameters:
		none
	*/
	
	reload: function(notoggle) {
		if (this.data.load) {
			this.loading = false;
			this.load(this.data.load,notoggle);
		}
	},
	...
	*/
	...
	Parameters:
		...
		notoggle - does not hide the tree while reloading
	*/
	
	load: function(url, notoggle) {
	...
	_loaded: function(text, xml) {
		...
		if(xml==null) {
			...
			window.location=rfstring_SITEPREFIX + '/Logout';
		}
		...
		for (var i=0; i<n.length; i++) if (n[i].tagName == 'node') {
			...
			if (node.data.load) {
				...
				node.onExpand = function(state) {
					...
					if (!this.forceReload) this.onExpand = new Function();
				}
			}
